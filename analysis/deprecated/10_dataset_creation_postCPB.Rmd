...
```{r}
abp_mean_expanded_postcpb<-inner_join(  abp_mean_expanded,  AllPeriodsDF)%>%
...
abp_mean_expanded_postcpb_count<-abp_mean_expanded_postcpb %>%
  group_by(Case_Name) %>%
  mutate(
    ones = 1,
    counter = cumsum(ones)
...
abp_mean_reshaped_postcpb<-dcast(abp_mean_expanded_postcpb_count, Case_Name ~ counter, value.var = "Value_imputed")
```
...
```{r}
abp_mean_reshaped_postcpb[is.na(abp_mean_reshaped_postcpb)]<-(-1)
```

### Entropy

```{r}
abp_mean_reshaped_postcpb['entropy']<-round(apply(abp_mean_reshaped_postcpb[2:ncol(abp_mean_reshaped_postcpb)], 1 , entropy ) , 2)
```
...
```{r}
abp_mean_postcpb_all_data<-merge(abp_mean_reshaped_postcpb,
                         FinalHemodynamicsDataset%>%
                          dplyr::select( Case_Name
                                        ...
                                        ,Q_AUC65MAP_postCPB_mmHgmin
                                        ,Q_TWA65MAP_postCPB_mmHg
...
```{r}
#abp_mean_postcpb_all_data <- abp_mean_postcpb_all_data[,colSums(is.na(abp_mean_postcpb_all_data))<nrow(abp_mean_postcpb_all_data)]
```
...
```{r}
abp_mean_postcpb_all_data$mean_ef_median_imp[is.na(abp_mean_postcpb_all_data$mean_ef_median_imp)]<-round(mean(abp_mean_postcpb_all_data$mean_ef_median_imp,na.rm = T),2)
```
...
view(dfSummary(abp_mean_postcpb_all_data))
```

### Exporting df

```{r}
#write.csv(abp_mean_postcpb_all_data,'abp_mean_postcpb_all_data.csv',row.names = F)
``` 
